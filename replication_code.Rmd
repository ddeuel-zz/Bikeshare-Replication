...
```{r}
# all r comments beginnging with * are by Drake Deuel for paper replication Gov 1006

# *downloads and reads in data from csv files
...
# *plot of all data faceted by city
...
#### Figure 1. 
```{r}
# *focusing on data just around strike
...
```{r}
# *calculating mean and standard deviation for trips and covariates
...
```{r}
# *focusing just on Philly and looking at how long the trips are
...
```{r}
# *creating seperate datasets for each city
##All data
...
```{r}
# *looking at time periods around strike
##Set pre- and post-periods
